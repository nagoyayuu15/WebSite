...
let identity_done = fetch("./updatable/identity.html?"+now).then(data=>data.text()).then(html=>{
    identity.innerHTML = html
})

let hobby = document.querySelector(".feed_hobby .main")
let hobby_done = fetch("./updatable/hobby.html?"+now).then(data=>data.text()).then(html=>{
    hobby.innerHTML = html
})

let comment = document.querySelector(".feed_comment .main")
let comment_done = fetch("./updatable/comment.html?"+now).then(data=>data.text()).then(html=>{
    comment.innerHTML = html
})

let history = document.querySelector(".feed_carrier>ul")
let circle_svg=fetch("./circle.svg").then(data=>data.text())
let history_recodes = fetch("./updatable/history.json?"+now).then(data=>data.text()).then(json=>JSON.parse(json))
